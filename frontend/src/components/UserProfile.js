...
const UserProfile = ({ loggedInUser, navigateTo }) => {
  ...
  const handleUpdateProfile = async () => {
    try {
      const updatedProfile = await updateUserProfile(profileData._id, formData);
  ...
  const handleDeleteProfile = async () => {
    try {
      await deleteUserProfile(profileData._id);
  ...
  const handleForgotPassword = async () => {
    try {
      const response = await forgotPassword(profileData.email);
  ...
  const handleBack = () => {
    navigateTo("adminDashboard"); // Navigate back to the Admin Dashboard
  };
